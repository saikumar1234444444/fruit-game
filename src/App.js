...
class App extends Component {
  state = {activeTab: tabsList[0].tabId, click: 0, score: 0, seconds: 60}
  ...
  clickImage = () => {
    this.setState({click: Math.ceil(Math.random() * 29)})
  ...
  getFilterData = () => {
    ...
    const data = imagesList.filter(simple => simple.category === activeTab)
    return data
  }

  render() {
    const data = this.getFilterData()
    const {click, score, seconds} = this.state
    return (
      <div>
        <nav className="nav-container">
          ...
          <div className="timer-container">
            ...
            <img
              ...
              className="picture"
            />
            <p className="heading1">{seconds} secs</p>
          </div>
        </nav>
        ...
        <div className="img-container">
          {seconds === 0 ? (
            ...
            <>
              <img
                src={imagesList[click].imageUrl}
                alt="match"
                className="match-image"
              />
              <ul className="ul-container">
                {tabsList.map(each => (
                  <TabItem each={each} clickOnTab={this.clickOnTab} />
                ))}
              </ul>
